function schemaball4(strNames, corrMatrix, fontsize, positive_color_hs, negative_color_hs,theta,scaleFactorCol, scaleFactorLine )
%% SCHEMABALL(strNames, corrMatrix, fontsize, positive_color_hs, negative_color_hs, theta)%theta
%	edited by Adam Steel, March 2015.
...
%
	...
	%end
	if nargin<5 || isempty(positive_color_hs)
        disp('true')
	...
	for ii=idx'
		...
		if corrMatrix(jj,kk)>=0
            %disp([positive_color_hs scaleFactorCol*abs(corrMatrix(jj,kk)) 1])
            
			%clr = hsv2rgb([positive_color_hs 1]);
			clr = hsv2rgb([positive_color_hs scaleFactorCol*abs(corrMatrix(jj,kk)) 1]);
        else
            %clr = hsv2rgb([negative_color_hs 1]);
			clr = hsv2rgb([negative_color_hs scaleFactorCol*abs(corrMatrix(jj,kk)) 1]);
        end
        %disp(scaleFactorCol*abs(corrMatrix(jj,kk)))
        if corrMatrix(jj,kk)~=0
            plot(Bt(:,1),Bt(:,2),'color',clr,'LineWidth',abs(corrMatrix(jj,kk)^2*scaleFactorLine));%,'LineSmoothing','on');
		end
		%disp(abs(corrMatrix(jj,kk))*sqrt(abs(corrMatrix(jj,kk))*scaleFactorLine))
	end
	...
	for ii=1:M
		rectangle('Curvature',[1 1],'edgeColor',clr(ii,:),'FaceColor',clr(ii,:),...
